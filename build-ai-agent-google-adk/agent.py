    ...
    def __init__(self):
        super().__init__(
            ...
            description="""
                ...
                If passes, respond "ok". Else respond "retry" with the specific
                fixes.
            """,
            output_key="validation_result",
        )
...
planner_tool = agent_tools.AgentTool(agent=robust_blog_planner)
writer_tool = agent_tools.AgentTool(agent=robust_blog_writer)
...
agent = Agent(
    ...
    instructions=f"""
        ...
        If the trends tool fails or times out, continue with a
        sensible outline and draft anyway.
